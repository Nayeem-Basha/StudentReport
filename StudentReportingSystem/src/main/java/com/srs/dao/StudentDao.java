...
public class StudentDao {
		...
		return jdbctemplate.query("select*from srs.registor", new ResultSetExtractor<List<Student>>() {
			...
			public List<Student> extractData(ResultSet rs) throws SQLException, DataAccessException {
				...
				while(rs.next()) {
					...
					students.setPercentage(rs.getString("percentage"));
	...
	public boolean saveStudentsToStudentsTable(Student student) {
		...
		String sql="insert into srs.registor(id,name,semester,english,maths,science,percentage)"+"values("+student.getId()+",'"+student.getName()+"','"
		+student.getSemester()+"','"+student.getEnglish()+"','"+student.getMaths()+"','"+student.getScience()+"','"+student.getPercentage()+"')";
				try {
